          ...
          monaco.languages.typescript.typescriptDefaults.addExtraLib(
            `declare module 'tana/block' {
              export const block: {
                 ...
                 * @param contractIds Single contract ID or array of up to 10 IDs
                 * @returns Single contract or array of contracts (null if not found)
                 * @throws Error if querying more than MAX_BATCH_QUERY items
                 */
                getContract(contractIds: string): Promise<Contract | null>;
                getContract(contractIds: string[]): Promise<(Contract | null)[]>;
              };
