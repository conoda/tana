...
<div class="state-viewer">
  ...
  <div class="tab-content">
    ...
    {#if activeTab !== 'results' && loading && !blockchainState}
        ...
        <div class="data-list">
          ...
          {:else}
            {#each blockchainState?.balances || [] as balance}
              {@const owner = blockchainState?.users?.find(u => u.id === balance.ownerId)}
                ...
                <div class="item-details">
                  {#if owner}
                    <div class="detail-row">
                      ...
                      <span class="value">{owner.username} ({owner.displayName})</span>
                    </div>
                  {:else}
                    <div class="detail-row">
                      <span class="label">Owner ID:</span>
                      <span class="value mono">{balance.ownerId.substring(0, 16)}...</span>
                    </div>
                  {/if}
                  <div class="detail-row">
            ...
            {#each blockchainState?.transactions || [] as tx}
              <div class="data-item">
                <div class="tx-header">
                  ...
                  <span class="tx-status" class:pending={tx.status === 'pending'} class:confirmed={tx.status === 'confirmed'}>
                    {tx.status}
                  </span>
                </div>
                <div class="item-details">
                  {#if tx.amount && tx.currencyCode}
                    <div class="detail-row">
                      ...
                      <span class="value tx-amount-value">{formatAmount(tx.amount)} {tx.currencyCode}</span>
                    </div>
                  {/if}
                  <div class="detail-row">
                    <span class="label">From:</span>
                    <span class="value mono">{tx.from.substring(0, 16)}...</span>
                  </div>
                  <div class="detail-row">
                    <span class="label">To:</span>
                    <span class="value mono">{tx.to.substring(0, 16)}...</span>
                  </div>
                  {#if tx.blockHeight}
                    <div class="detail-row">
                  ...
                  {#if tx.contractId}
                    <div class="detail-row">
                      ...
                      <span class="value mono">{tx.contractId.substring(0, 16)}...</span>
                    </div>
                  {/if}
                  <div class="detail-row">
                    <span class="label">Created:</span>
                  ...
                  {#if tx.confirmedAt}
                    <div class="detail-row">
                      ...
                      <span class="value">{formatDate(tx.confirmedAt)}</span>
                    </div>
                  {/if}
                </div>
              </div>
