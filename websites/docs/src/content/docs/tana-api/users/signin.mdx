---
title: /users/signin
description: documentation for the endpoint
---

when given a user identifier (username, email, or phone number) and password as input, this endpoint will return a token if the information is valid

<details>
 <summary><code style="color:#49CC90">POST</code> <code><b>/users/signin</b></code> <code>(authenticate user)</code></summary>

### Request

```javascript
curl https://api.conoda.co/users/signin
```

### Parameters

<table>
 <tr>
    <td>_parameter_</td>
    <td>_type_</td>
    <td>_necessity_</td>
  </tr>
 <tr>
    <td>**user**</td>
    <td>string</td>
    <td>required</td>
  </tr>
 <tr>
    <td>**password**</td>
    <td>string</td>
    <td>required</td>
  </tr>
</table>

### Response

<table>
 <tr>
    <td>http code</td>
    <td>content-type</td>
    <td>body</td>
  </tr>
  <tr>
    <td>200</td>
    <td>application/json</td>
<td>

```json
{
  "conoda/addis": "v1.0.0",
  "api": "v1",
  "location": "seattle",
  "server-id": "9x93jv35",
  "rate-limit-remaining": 1000,
  "rate-limit-reset": 1684830400,
}
```

</td>
  </tr>
</table>

### Access Level

- [ ] Public
- [ ] Community
- [ ] Team
- [x] Self
- [x] Admin

</details>

### Method Not Allowed

sending a request with the following methods to this endpoint will return an 405 error

<code style="color:#61AFFE">GET</code> <code style="color:#FCA130">PUT</code> <code style="color:#F93E3E">DELETE</code>
